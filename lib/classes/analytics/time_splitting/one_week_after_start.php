 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class one_week_after_start extends \core_analytics\local\time_splitting\base {
    ...
    public function is_valid_analysable(\core_analytics\analysable $analysable) {
        ...
        if ($now < $this->get_prediction_time($analysable)) {
            // We can not use this to get predictions as we have not reached start + 1 week.
            return false;
        }
    ...
    private function get_prediction_time(?\core_analytics\analysable $analysable = null) {
        ...
        $predictiontime->add(new \DateInterval('P1W'));
