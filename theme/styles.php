...
if (($fallbacksheet = theme_server_fallback_content($theme, $themesubrev, $rev)) && !$theme->has_css_cached_content()) {
...
if ($sendaftergeneration || $lock) {
    ...
    if (!file_exists($chunkedcandidatesheet)) {
        ...
        $candidatesheet = theme_styles_generate_and_store($theme, $themesubrev, $rev, $candidatedir);
    }
 ...
 */
function theme_styles_generate_and_store($theme, $themesubrev, $rev, $candidatedir) {
    global $CFG;
    ...
    if (!empty(min_get_slash_argument())) {
        if ($theme->use_svg_icons()) {
            $chunkurl = "{$relroot}/theme/styles.php/{$theme->name}/{$rev}/$candidatename";
        } else {
            $chunkurl = "{$relroot}/theme/styles.php/_s/{$theme->name}/{$rev}/$candidatename";
        }
    } else {
        if ($theme->use_svg_icons()) {
            $chunkurl = "{$relroot}/theme/styles.php?theme={$theme->name}&rev={$rev}&type=$candidatename";
        } else {
            $chunkurl = "{$relroot}/theme/styles.php?theme={$theme->name}&rev={$rev}&type=$candidatename&svg=0";
        }
    }
 ...
 */
function theme_server_fallback_content($theme, $themesubrev, $rev) {
    global $CFG;
    ...
    $filename = theme_styles_get_filename($type, $themesubrev, $rev);
 ...
 */
function theme_styles_get_filename($type, $themesubrev, $usesvg, $chunk = null) {
