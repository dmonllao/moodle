...
class potential_cognitive_depth extends \core_analytics\local\indicator\linear {
    ...
    public static function get_name() {
        return get_string('indicator:potentialcognitive', 'moodle');
    }
    ...
    public function calculate_sample($sampleid, $sampleorigin, $notusedstarttime = false, $notusedendtime = false) {
        ...
        // -1 to 1 range split in 5 parts (the max cognitive depth level).
        // Note that we divide by 4 because we start from 0.
        $levelscore = round((self::get_max_value() - self::get_min_value()) / 4, 2);
        // We rest $levelscore because we want to start from the lower score and there is no cognitive depth level 0.
