            ...
            if (empty($results->error) || (!empty($results->error) && $results->error->code !== 404)) {
                if (!empty($results->error)) {
                    ...
                    if (empty($results->field) || !isset($results->field->type) ||
                        ...
                        if (($results->field->type === 'int' && $expectedsolrfield == 'pint') ||
                                ($results->field->type === 'tdate' && $expectedsolrfield == 'pdate') ||
                                ($results->field->type === 'pint' && $expectedsolrfield == 'int') ||
                                ($results->field->type === 'pdate' && $expectedsolrfield == 'tdate')) {
                            // Specific error for solr field type mismatch.
                            throw new \moodle_exception('errorfieldtypemismatch', 'search_solr', '',
                                get_string('schemafieldautocreated', 'search_solr', $fieldname));
